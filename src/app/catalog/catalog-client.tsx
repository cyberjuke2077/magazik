...
export function CatalogClient({ categories, products, totalProducts }: CatalogClientProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return r
  }, [products, selectedCategory, selectedBrands, searchQuery, sort, inStockOnly, priceMin, priceMax])

  // Find active category from hierarchy
  const activeCategory = useMemo(() => {
    if (!selectedCategory) return null
    for (const cat of categories) {
      if (cat.slug === selectedCategory) return { name: cat.name, slug: cat.slug }
    ...
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <main>
        ...
        <div className="mx-auto max-w-[1400px] px-4 py-4">
          ...
          {/* Quick category filters - показываем только если есть товары */}
          {products.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              {categories.map((cat) => {
                ...
                const categoryProducts = products.filter((p) => {
                  ...
                  return cat.children.some(child => child.slug === p.categorySlug)
                })
