...
export function AccountClient() {
  ...
  return <div className="mx-auto max-w-3xl px-4 py-12">
    ...
    {!ready ? <p>Загрузка...</p> : requests.length ? <ul className="space-y-3">{requests.map((request) =>
      <li key={`${request.kind}:${request.id}`} className="rounded border p-4 break-all"><Link className="text-azure underline" href={requestStatusPath(request)}>
        {request.kind === 'wholesale' ? 'Оптовая заявка' : 'Заявка на КП'} {request.id}</Link><p className="text-sm">{new Date(request.createdAt).toLocaleDateString('ru-RU')}</p></li>)}</ul>
