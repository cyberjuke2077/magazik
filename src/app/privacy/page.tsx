...
const sections = [
  ...
  },
  {
    ...
    content: [
      'Текущая инфраструктура базы заявок размещена в регионе ЕС (Франция). Российский контур хранения ещё не подключён. Публичный запуск приёма заявок требует завершения переноса и проверки условий обработки данных.',
  ...
  },
  {
    ...
    content: [
      ...
      'Если на сайте будут подключены платёжные системы, порядок обработки платёжных данных будет описан до начала их использования.',
    ],
  },
  {
  ...
  {
    title: '7. Cookie-файлы и хранилище браузера',
    content: [
      'Корзина и ссылки на последние заявки хранятся в localStorage браузера. Историю ссылок можно удалить на странице «Ваши заявки».',
      'В sessionStorage текущей вкладки сохраняются контактные данные и содержимое черновика, а для проверки незавершённой отправки - случайный ключ, хеш и снимок исходной заявки. Эти данные хранятся не более 2 часов. Ключ и снимок завершённой операции очищаются после подтверждения, а несохранённые изменения могут остаться отдельным черновиком до истечения срока.',
      'Cookie используются для сессии администратора.',
