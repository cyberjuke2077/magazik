...
class DigiKeyExtractor(BaseExtractor):
    """Extract data from Digi-Key.com."""
    
    def __init__(self, name: str = "digikey"):
        """Initialize Digi-Key extractor."""
        super().__init__(name)
        self.base_url = "https://www.digikey.com"
    
    def get_url(self, part_number: str, manufacturer: str) -> str:
        """Get Digi-Key search URL."""
        return f"{self.base_url}/en/products/result?keywords={part_number}"
    ...
    async def extract(
        ...
        """Extract product data from Digi-Key."""
        try:
            ...
            try:
                await page.wait_for_selector(".product-details, .no-results", timeout=10000)
            ...
            if no_results:
                return ExtractResult.failure(f"Product {part_number} not found on Digi-Key")
            ...
            return ExtractResult.failure(f"Digi-Key extraction error: {str(e)}")
    ...
    async def _extract_product_data(
        ...
        try:
            ...
            name_elem = await page.query_selector(".product-title, .pdp-product-name")
            name = await name_elem.inner_text() if name_elem else part_number
            
            # Extract description
            desc_elem = await page.query_selector(".product-description")
            description = await desc_elem.inner_text() if desc_elem else ""
            
            # Extract manufacturer
            mfr_elem = await page.query_selector(".manufacturer-name")
    ...
    async def _extract_specifications(self, page: Page) -> Dict[str, str]:
        ...
        try:
            ...
            spec_rows = await page.query_selector_all(".specifications-table tr, .product-attributes tr")
    ...
    async def _extract_images(self, page: Page) -> List[str]:
        ...
        try:
            img_elems = await page.query_selector_all(".product-image img")
