 ...
 * 
 * Usage:
 *   node scripts/convert-proxies.js /path/to/webshare-proxies.txt
 */

import { readFileSync, writeFileSync } from 'fs'
import { loadWebshareProxies, formatProxyUrl } from '../src/lib/proxy/index.js'

function main() {
  const args = process.argv.slice(2)
  
  if (args.length === 0) {
    console.error('Usage: node scripts/convert-proxies.js <proxy-file>')
    console.error('Example: node scripts/convert-proxies.js ~/Downloads/webshare-proxies.txt')
    process.exit(1)
  }
  ...
  try {
    ...
    console.log('2. Run parser with proxies: node scripts/parse-with-proxies.js')
    console.log('')
