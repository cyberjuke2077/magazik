...
## Надёжность инструментов и проверок

- Пути с `()` и `[]` передавать в shell отдельными quoted arguments: zsh иначе
  интерпретирует их как glob или синтаксис команды.
- Временные TypeScript-скрипты с `await` запускать через `async main()` с явным
  `catch` и `process.exitCode = 1`: top-level await не работает в CJS-контексте.
- Перед запуском browser-act сверять `--help` установленной версии. Не переносить
  команды из документации другой версии без проверки.
- В Codex sandbox production build запускать через `npm run build -- --webpack`:
  Turbopack может падать при попытке открыть внутренний порт. Результат такой
  проверки указывать как webpack build, не как проверку Turbopack.
- В Playwright после навигационного клика ждать конкретный URL или состояние
  назначения перед следующим кликом. Последовательные `click()` сами по себе
  не подтверждают завершение клиентской навигации.
- Async-тесты cart storage обязаны ждать Promise мутации и итоговое состояние
  через `waitFor` или соответствующий async assertion.
- Перед обновлением визуальных эталонов проверять параметры Playwright по
  установленным типам и запускать TypeScript. Для CSS в `toHaveScreenshot`
  использовать `stylePath`; затем просматривать PNG, а не полагаться только
  на зелёный тест: неизвестный параметр может быть молча проигнорирован.
- Для полной замены файла через `apply_patch` использовать один `Update File`.
  Не совмещать `Delete File` и `Add File` одного пути в одном patch.
