...
13. Для браузерной автоматизации и browser QA использовать Playwright MCP.
...
6. Сессия с материальными изменениями не заканчивается локально: обновить второй мозг,
   сделать поимённый commit, отправить текущую ветку и создать или обновить один
   draft PR в `main`. Если push или PR заблокирован, указать точную причину в handoff.
7. Документы, планы, ретроспективы, repo-skills и инструкции агентов синхронизируются
   тем же PR, что и код. Секреты, `.env`, модели, локальные БД, browser profiles,
   логи и исходные файлы заказчика в Git не добавлять.
