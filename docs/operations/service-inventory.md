...
|---|---|---|---|---|---|
| GitHub `cyberjuke2077/magazik` | Исходный код, ветки, PR | `cyberjuke2077` | Write для Lunar, review и merge для владельца | `verified`: PR #14 слит merge commit `4f0575d`, RLS PR #15 - `5d10ecf`; enrichment PR #13 остаётся отдельным | Продолжать task branch -> PR -> merge владельца; PR #13 не смешивать с deployment cleanup |
| Vercel `cyberjuke2077s-projects/electromagaz-production`, project `prj_RkTeKu3bIIkImfBTfU11zTzpw8bm` | Production deploy | Аккаунт `cyberjuke2077` | Owner текущего Hobby team | `verified`: deployment `dpl_AXPeVvrC9c9seZzoJBgYQjGTPnFw` после PR #15 имеет `Ready`; alias, страницы, карточка, export, API и database health проверены 2026-08-20 | Сохранять Production env только на этом target; старый `magazik` не удалять до отдельной cleanup-задачи |
