...
Приоритет решений: [итоги выпуска](release-closeout-2026-09-20.md).
Выпуск в существующий Vercel/Supabase разрешён и выполнен. Перенос в РФ
остаётся отложенным. Неперепроверенные внешние сервисы сохраняют прежние
датированные статусы; значения env в документы не копируются.
...
|---|---|---|---|---|---|
| GitHub `cyberjuke2077/magazik` | Исходный код, ветки, PR | `cyberjuke2077` | Write для Lunar, review/merge владельца | `verified`: PR #26 слит, `ae1f732`; исходная ветка сохранена | Продолжать от свежего origin/main |
| Vercel `cyberjuke2077s-projects/electromagaz-production`, project `prj_RkTeKu3bIIkImfBTfU11zTzpw8bm` | Production storefront | Аккаунт `cyberjuke2077` | Owner текущего Hobby team | `verified`: READY на `ae1f732`, публичные маршруты, health и admin login/logout проверены | См. точные deployment ID в итогах выпуска |
| Supabase `37Lunar's Org / 37Lunar's Project`, ref `dbumwpnbtvixfusxnggn` | Production PostgreSQL, заявки и лиды | Owner `37Lunar`, Administrator `cyberjuke2077` | Свои аккаунты без передачи credentials | `verified`: backup/restore PostgreSQL 17, 12 миграций, runtime CRUD и anonymous denial | Сохранить backup вне Git; не использовать production как sandbox |
...
| Telegram Bot API | Уведомления менеджеру | Владелец Electromagaz | Bot token и chat ID | `partial`: Production env и Vault готовы, cron выключен; сообщение не отправлено | Ожидается уточнение разрешения на штатную обработку очереди, см. итоги выпуска |
...
- Миграция `20260917233000_ensure_runtime_role_grants` создаёт отсутствующую
  ...
  `AdminSession`. У безопасной роли ALTER пропускается; опасные role flags требуют прав
  для снятия. Миграция завершается ошибкой, если роль
