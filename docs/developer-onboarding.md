...
Для совместной работы владелец приглашает разработчика в общую организацию
Supabase с ролью `Developer`. После принятия приглашения разработчик получает
строку Session Pooler из Supabase Dashboard и добавляет её в свой локальный
`.env` как `DATABASE_URL` и `DIRECT_URL`. Пароль и полный URL не отправляются
в Git, чат, issue или PR.
