...
## Известные долги/находки (на 2026-06-14)

- **wholesale-форма — пустышка**: собирает ПДн оптовика, показывает «успех», но
  данные никуда не уходят. Чинить: server action + Telegram/БД + чекбокс согласия.
  Развилка: где хранить оптовые лиды (отдельная модель vs переиспользовать QuoteRequest).
...
- Заявления о сертификации (РСТ/ГОСТ) и «с 2012 / 12+ лет» — проверить достоверность.
...
- [ ] Чекбокс согласия ПДн НЕ предзаполнен, ссылки на /privacy и /offer живые
